...
module.exports = function (lines, initialized) {
	...
	me.renderMovie = function (screenplay, filename, callback) {
		...
		var img = new png.Image(c.width, c.height, new Buffer(c.width*c.height*3));
		...
		function checkTime() {
			...
			time = time/1000;
			console.log('   time left: '+Math.floor(time/60)+':'+(100 + time % 60).toFixed(0).slice(-2));
			lastTime = now;
		}
	...
	me.getHash = function (screenplay) {
		var states = [];
		screenplay.forEach(function (value) {
			if (typeof value != 'number') {
				states.push(calcState(value));
			} else {
				states.push(value);
			}
		})
		...
		shasum.update(JSON.stringify(states), 'utf8');
		shasum = shasum.digest('base64');
		shasum = shasum.replace(/[\+\=]+/g, '_').substr(0,16);
		shasum = shasum.replace(/[\/]+/g, '$').substr(0,16);
		return shasum;
	}
	...
	function initFont(name, callback) {
		...
		var result = {
			...
			lookup:      function (char) {
				...
				if (result < 0) {
					console.error('ERROR "'+char+'" ('+char.charCodeAt(0)+') not found in "'+letters+'"');
					result = letters.indexOf('?');
				}
				return result;
			}
		}
